...
def _check_and_graduate(enrollment, total_lessons, total_tests, update_fields):
    finished_lessons = min(update_fields.get('finished_darslar_soni', enrollment.finished_darslar_soni), total_lessons)
    finished_tests = min(update_fields.get('finished_test_soni', enrollment.finished_test_soni), total_tests)
    update_fields['finished_darslar_soni'] = finished_lessons
    update_fields['finished_test_soni'] = finished_tests

    is_now_completed = total_lessons > 0 and finished_lessons >= total_lessons and finished_tests >= total_tests
...
def _schedule_certificate(user_id, course_id):
    def _create():
        try:
            Certificate = apps.get_model('certificate', 'Certificate')
            cert, created = Certificate.objects.get_or_create(
                user_id=user_id, course_id=course_id, defaults={'status': 'pending'},
            )
            logger.info(f"Certificate {'yaratildi' if created else 'mavjud'}: user={user_id}, course={course_id}")
        except Exception as e:
            logger.error(f"_schedule_certificate xato: {e}", exc_info=True)
...
def on_lesson_status_save(sender, instance, **kwargs):
    if not instance.is_completed or not (instance.user_id and instance.lesson_id):
        return
...
def on_test_session_save(sender, instance, **kwargs):
    if instance.status != 'completed' or not (instance.user_id and instance.test_id):
        return
    ...
    def _run(user_id=instance.user_id, test_id=instance.test_id):
        try:
            ...
            finished_tests = TestSession.objects.filter(
