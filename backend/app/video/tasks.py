...
def download_from_minio(storage_field, local_path: str):
    """storage field orqali — 'media/' prefix avtomatik"""
...
def convert_to_hls(self, movie_id: int):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # ✅ storage field orqali yuklab olish
            ...
            s3_hls_prefix = f"media/movies/hls/{movie_id}"  # ✅ "media/" prefix bilan
        ...
        Video.objects.filter(id=movie_id).update(hls_url=hls_url)
...
@shared_task(bind=True, max_retries=3)
def generate_thumbnail(self, movie_id: int, time_sec: int = 30):
    from .models import Video
    try:
        movie = Video.objects.get(id=movie_id)
